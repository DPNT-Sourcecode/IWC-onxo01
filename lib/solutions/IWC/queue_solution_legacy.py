...
class Queue:
    ...
    def _earliest_group_timestamp_for_task(task):
        ts= task.metadata.get("group_earliest_timestamp", MAX_TIMESTAMP)
    ...
    def enqueue(self, item: TaskSubmission) -> int:
        ...
        for task in tasks:
            ...
            self._queue= [
                existing for existing in self._queue
                if not (existing.user_id == task.user_id and existing.provider == task.provider)
    ...
    def dequeue(self):
        ...
        def sort_task(task):
            ...
            task_internal_age= (now-timestamp).total_seconds()
            is_bank_old= is_bank and task_internal_age >= 300
            apply_pen_bank= (
                is_bank and not is_bank_old
                and timestamp>global_earliest_stamp
            )
            bank_penalty=1 if apply_pen_bank else 0
            priority_rank= 0 if is_bank_old else int(priority.value)
